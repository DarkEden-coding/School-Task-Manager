(() => {
  ...
  function shell(view) {
    const titles = { dashboard: 'Dashboard', classes: 'Classes', calendar: 'Calendar review', imports: 'Import', settings: 'Settings' };
    const nav = [['dashboard','Dashboard'],['classes','Classes'],['calendar','Calendar review'],['imports','Import'],['settings','Settings']];
  ...
  async function navigate(view) { current=view; clearInterval(poller); app.innerHTML=shell(view); bindShell(); try { if(view==='dashboard') await schoolDashboard(); if(view==='classes') await classesPage(); if(view==='calendar') await calendarReview(); if(view==='imports') await importsPage(); if(view==='settings') await settings(); } catch(e) { setView(`<div class="card notice error">${esc(e.message)}</div>`); } }
  ...
  async function schoolDashboard() { await loadSchool(); const term=activeTerm(), today=localDay(new Date(),true), week=new Date(today); week.setDate(today.getDate()+7); const classes=schoolData.classes.filter(c=>term&&String(c.termId)===String(term.id)); const open=openAssignments(); const upcoming=open.filter(a=>dueDate(a)).sort((a,b)=>dueDate(a)-dueDate(b)); const groups={Overdue:[],Today:[],Tomorrow:[]}; for(let i=2;i<=7;i++) groups[new Date(today.getTime()+i*86400000).toLocaleDateString(undefined,{weekday:'long',month:'short',day:'numeric'})]=[]; upcoming.forEach(a=>{const d=localDay(dueDate(a),true); let key=d<today?'Overdue':d.getTime()===today.getTime()?'Today':d.getTime()===today.getTime()+86400000?'Tomorrow':d.toLocaleDateString(undefined,{weekday:'long',month:'short',day:'numeric'}); if(groups[key])groups[key].push(a);});
    const cards=classes.map(c=>{const work=open.filter(a=>String(a.classId)===String(c.id));const next=work.filter(a=>dueDate(a)).sort((a,b)=>dueDate(a)-dueDate(b))[0]; const n=work.filter(a=>{let due=dueDate(a),d=due&&localDay(due,true);return d&&d>=today&&d<week;}).length;return `<article class="card class-card"><h2>${esc(c.name)}</h2><p>${esc(c.instructor||'Instructor TBD')}</p><strong>${next?esc(next.title):'No upcoming assignment'}</strong><small>${next&&dueDate(next)?esc(dueDate(next).toLocaleString(undefined,{timeZone:schoolTimezone})):''} · ${n} due this week</small><button class="text-button" data-class="${esc(c.id)}">Open class</button></article>`;}).join('')||'<div class="empty">Add a class to start planning this term.</div>';
    const list=Object.entries(groups).map(([k,v])=>v.length?`<section class="upcoming-group"><h3>${esc(k)}</h3>${v.map(a=>assignmentRow(a)).join('')}</section>`:'').join('')||'<div class="empty">No upcoming work in the next week.</div>';
    setView(`<div class="section-head"><div><h2>${esc(term?.name||'No active term')}</h2><p class="muted">Your active term at a glance.</p></div><div class="actions">${button(calendarMode==='list'?'Calendar':'List','ghost','data-calendar-toggle')}${button('New assignment','primary','data-new-assignment')}</div></div>${calendarMode==='calendar'?renderAssignmentCalendar():`<div class="class-cards">${cards}</div><section class="card section"><h2>Upcoming</h2>${list}</section>`}`); document.querySelector('[data-calendar-toggle]').onclick=()=>{calendarMode=calendarMode==='list'?'calendar':'list';schoolDashboard();}; document.querySelectorAll('[data-class]').forEach(b=>b.onclick=()=>classDetail(b.dataset.class)); document.querySelector('[data-new-assignment]')?.addEventListener('click',()=>schoolData.classes.length?assignmentForm():notify('Create a term and class first.',true)); bindCompletion(); if(calendarMode==='calendar')bindCalendar(); }
  ...
  function importItem(item) { const p=item.payload||{}, noop=item.action==='noop'||item.action==='duplicate'||item.duplicate; return `<article class="import-item ${noop?'import-noop':''}"><label><input type="checkbox" data-import-select="${esc(item.id)}" ${item.needsReview||noop?'':'checked'}> <strong>${esc(item.kind||'item')}</strong> · ${esc(noop?'safe no-op duplicate':item.action||'add')}</label>${item.conflicts?.length?`<p class="notice error">${esc(item.conflicts.join(' · '))}</p>`:''}<textarea data-import-payload="${esc(item.id)}" rows="4">${esc(JSON.stringify(p,null,2))}</textarea></article>`; }
  async function importsPage(){const logs=await api('/api/school/imports').catch(()=>[]);setView(`<section class="card"><div class="section-head"><div><h2>Import school materials</h2><p class="muted">Paste a syllabus or schedule, add screenshots, and tell us how to interpret them.</p></div></div><form id="import-form"><div class="field"><label>Paste text<textarea name="text" rows="7" placeholder="Paste syllabus, assignments, dates, or class details"></textarea></label></div><div class="form-grid"><div class="field"><label>Images (up to 8)<input name="images" type="file" accept="image/*" multiple></label></div><div class="field"><label>One-off instruction<input name="instructions" placeholder="e.g. only import deadlines"></label></div></div>${button('Create proposal','primary','type="submit"')}</form><p class="muted">Source contents are deleted after approval.</p></section><section class="card section"><h2>Recent imports</h2>${logs.length?logs.map(x=>`<div class="work-row"><span>${esc(x.inputMethod||'import')} · ${esc(formatDate(x.createdAt))}</span><span class="actions"><span class="tag">${esc(x.status)}</span>${x.status==='pending'?button('Review','ghost',`data-import-open="${esc(x.id)}"`):''}</span></div>`).join(''):'<p class="muted">No imports yet.</p>'}</section>`);document.querySelectorAll('[data-import-open]').forEach(b=>b.onclick=async()=>{try{renderImportProposal(await api(`/api/school/imports/${b.dataset.importOpen}`));}catch(e){notify(e.message,true);}});document.querySelector('#import-form').onsubmit=async e=>{e.preventDefault();const form=e.currentTarget, submit=form.querySelector('[type="submit"]');try{const files=Array.from(form.querySelector('input[name="images"]').files||[]);if(files.length>8)throw new Error('Choose no more than 8 images.');const f=new FormData();f.append('text',form.text.value);f.append('instructions',form.instructions.value);files.forEach(file=>f.append('images',file));submit.disabled=true;submit.textContent='Analyzing…';const result=await api('/api/school/imports',{method:'POST',body:f});renderImportProposal(result);}catch(x){notify(x.message,true);submit.disabled=false;submit.textContent='Create proposal';}};}
  function renderImportProposal(proposal){const items=proposal.items||[], safeNoops=items.filter(item=>item.action==='noop'||item.action==='duplicate'||item.duplicate), changes=items.filter(item=>!safeNoops.includes(item));setView(`<section class="card"><div class="section-head"><div><h2>Review import proposal</h2><p class="muted">Edit any item before applying selected changes. Source contents are deleted after approval.</p></div>${button('Discard','danger','data-import-discard')}</div>${changes.map(importItem).join('')||(!safeNoops.length?'<div class="empty">Nothing was found.</div>':'')} ${safeNoops.length?`<details class="section"><summary>Safe no-op duplicates (${safeNoops.length})</summary>${safeNoops.map(importItem).join('')}</details>`:''}<div class="actions section">${button('Apply selected','primary','data-import-apply')}</div></section>`);document.querySelector('[data-import-discard]').onclick=async()=>{await api(`/api/school/imports/${proposal.id}`,{method:'DELETE'});importsPage();};document.querySelector('[data-import-apply]').onclick=async()=>{try{const items=[...document.querySelectorAll('[data-import-select]:checked')].map(c=>{let raw=document.querySelector(`[data-import-payload="${c.dataset.importSelect}"]`).value;return {id:Number(c.dataset.importSelect),payload:JSON.parse(raw)};});await api(`/api/school/imports/${proposal.id}/apply`,{method:'POST',body:JSON.stringify({items})});notify('Selected changes applied.');navigate('classes');}catch(e){notify(`Could not apply proposal: ${e.message}`,true);}};}
